  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 gap-2">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <h3 className="text-xl font-bold tracking-tight leading-tight capitalize truncate text-gray-900 dark:text-white">
              {mainTitle}
            </h3>
            {item.customName && (
              <p className="text-xs font-medium text-gray-400 dark:text-gray-500 capitalize mt-0.5 truncate">
                {subTitle}
              </p>
            )}
          </div>
        ...
        {!item.isOwned && (
          <div className="shrink-0 text-right">
            <p className="text-lg font-bold text-gray-900 dark:text-white tracking-tight leading-none">
              {currency.symbol}{(item.rentalPrice || item.pricePerDay).toLocaleString()}
            </p>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <span className="w-28 flex justify-center px-2.5 py-1 rounded-lg text-xs font-bold capitalize tracking-wide bg-gray-100/50 dark:bg-white/5 text-gray-900 dark:text-white border border-gray-200/50 dark:border-white/10">
          {item.category}
        </span>
        <span className={`w-28 flex justify-center px-2.5 py-1 rounded-lg text-xs font-bold capitalize tracking-wide border ${item.isOwned
      ...
      <div className="mt-auto grid grid-cols-2 gap-2 bg-white/40 dark:bg-white/5 p-3 rounded-xl border border-white/40 dark:border-white/10">
        {Object.entries(item.specs).slice(0, 4).map(([key, val]) => (
          <div key={key} className="flex flex-col min-w-0 overflow-hidden">
            <span className="text-xs font-bold text-gray-400 dark:text-gray-500 capitalize tracking-wide leading-none mb-1 truncate">
              {key.replace(/([A-Z])/g, ' $1').trim()}
            </span>
            <span className="text-sm font-bold text-gray-900 dark:text-gray-200 truncate" title={String(val)}>
              {val}
            </span>
          </div>
        ))}
