                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-0.5">
                        <h4 className="font-bold text-sm tracking-tight text-gray-900 dark:text-white capitalize truncate">
                          {note.title}
                        </h4>
                        {/* Assuming 'priority' exists on Note type for this update */}
                        {(note as any).priority && (
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide border ${(note as any).priority === 'urgent' ? 'bg-red-50 dark:bg-red-500/10 border-red-100 dark:border-red-500/20 text-red-600 dark:text-red-400' : 'bg-blue-50 dark:bg-indigo-500/10 border-blue-100 dark:border-indigo-500/20 text-blue-600 dark:text-[#4E47DD]'
                            }`}>
                            {(note as any).priority}
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 truncate">{note.content}</p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col h-full">
                  <div className="flex-1 p-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className={`text-[9px] font-bold uppercase tracking-wide ${themeText.replace('text-', 'text-opacity-80 text-')}`}>
                        {contextLabel}
                      </span>
                    </div>
                    <h3 className="font-bold text-gray-900 dark:text-white text-sm mb-2">{note.title}</h3>
                    <p className="text-gray-500 dark:text-gray-400 text-[11px] line-clamp-3">{note.content}</p>
                  </div>

                  <div className="px-6 pb-6 mt-auto">
                    <div className="flex items-center gap-3">
                      <span className="text-[11px] font-bold text-gray-400 dark:text-gray-500 flex items-center gap-2 uppercase tracking-wide">
...

      {
        aggregatedNotes.length === 0 && (
          <div className="py-32 flex flex-col items-center justify-center select-none">
            ...
            <h3 className="text-xl font-bold text-gray-500 dark:text-gray-400 tracking-tight uppercase">No notes found</h3>
            <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mt-2">Adjust filters to see content in your repository</p>
          </div>
        )
      }
    </motion.div >
