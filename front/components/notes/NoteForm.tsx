...
export const NoteForm: React.FC<NoteFormProps> = ({
    ...
    return (
        <div className="space-y-6">
            {/* Title */}
            <div>
                <label className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 block ml-1">Title</label>
                <input
                    type="text"
                    value={form.title}
                    onChange={e => setForm({ ...form, title: e.target.value })}
                    className="w-full bg-white dark:bg-[#2C2C30] border-2 border-gray-100 dark:border-white/5 rounded-[20px] px-6 py-4 text-sm font-bold focus:outline-none focus:border-blue-500 dark:border-indigo-500 dark:focus:border-yellow-400 transition-all placeholder-gray-300 dark:placeholder-gray-500 text-gray-900 dark:text-white"
            ...
            {/* Linkage Section */}
            <div>
                <label className="text-sm font-bold text-gray-700 mb-2 block ml-1">Context link</label>
                <div className="flex gap-2 mb-3">
                    <button
                        onClick={() => setForm({ ...form, linkType: 'none', linkedId: '' })}
                        className={`flex-1 py-2 rounded-xl text-[9px] font-bold tracking-wide border transition-all ${form.linkType === 'none' ? 'bg-gray-900 border-gray-900 text-white dark:bg-white dark:border-white dark:text-black' : 'bg-white dark:bg-[#2C2C30] border-gray-100 dark:border-white/5 text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                    >
                        General
                    </button>
                    <button
                        onClick={() => setForm({ ...form, linkType: 'shot', linkedId: '' })}
                        className={`flex-1 py-2 rounded-xl text-[9px] font-bold tracking-wide border transition-all ${form.linkType === 'shot' ? 'bg-blue-600 dark:bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-[#2C2C30] border-gray-100 dark:border-white/5 text-gray-400 dark:text-gray-500 hover:bg-blue-50 dark:hover:bg-blue-500 dark:bg-indigo-500/10'}`}
                    >
                        Shot
                    </button>
                    <button
                        onClick={() => setForm({ ...form, linkType: 'task', linkedId: '' })}
                        className={`flex-1 py-2 rounded-xl text-[9px] font-bold tracking-wide border transition-all ${form.linkType === 'task' ? 'bg-orange-500 text-white border-orange-500' : 'bg-white dark:bg-[#2C2C30] border-gray-100 dark:border-white/5 text-gray-400 dark:text-gray-500 hover:bg-orange-50 dark:hover:bg-orange-500/10'}`}
                    >
                        Task
                    </button>
                </div>
                ...
                {form.linkType !== 'none' && (
                    <div className="relative animate-in slide-in-from-top-1">
                        ...
                        <select
                            ...
                            className="w-full appearance-none bg-white dark:bg-[#2C2C30] border-2 border-gray-100 dark:border-white/5 rounded-[20px] pl-12 pr-10 py-4 text-sm font-bold focus:outline-none focus:border-blue-500 dark:border-indigo-500 transition-all cursor-pointer text-gray-900 dark:text-white"
                        >
            ...
            {/* Content */}
            <div>
                <label className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 block ml-1">Content</label>
                <textarea
                    value={form.content}
            ...
            <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-2">
                        ...
                        <label className="text-[9px] font-bold text-gray-400 tracking-wide capitalize">Attachments</label>
                        <span className="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full text-[8px] font-bold">{form.attachments.length}</span>
                    </div>
                    <button
                        onClick={() => fileInputRef.current?.click()}
                        className="flex items-center gap-2 text-[9px] font-bold tracking-wide text-blue-600 dark:text-indigo-600 hover:text-indigo-700 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-colors"
                    >
                        ...
                        {form.attachments.map(att => (
                            <div key={att.id} className="group relative bg-white p-3 rounded-2xl border border-gray-100 flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[10px] font-bold text-gray-900 truncate">{att.name}</p>
                                    <p className="text-[8px] font-medium text-gray-400 tracking-wide">{att.size || 'Unknown Size'}</p>
                                </div>
