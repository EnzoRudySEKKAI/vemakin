        ...
        <div className="max-w-[1920px] ml-0 p-4 md:p-6 pt-10 grid grid-cols-1 xl:grid-cols-3 gap-8 pb-32">
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className="bg-white dark:bg-[#1C1C1E] border border-gray-100 dark:border-white/10 rounded-[28px] p-8 shadow-sm">
              <label className="text-[10px] font-bold text-gray-400 ml-1 uppercase tracking-wide block mb-3">Note Content</label>
              {isEditing ? (
                <div className="space-y-4">
                  <input
                    ...
                    className="w-full bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-white/10 rounded-2xl px-5 py-3 text-lg font-bold tracking-tight text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-300"
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{note.title || 'Untitled Note'}</h3>
            ...
            <div className="bg-white dark:bg-[#1C1C1E] border border-gray-100 dark:border-white/10 rounded-[28px] p-8 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white">Attachments</h4>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{editedNote.attachments?.length || 0} Files</p>
                  </div>
                </div>
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-gray-300 rounded-xl text-xs font-bold transition-all border border-gray-200 dark:border-white/10"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {editedNote.attachments && editedNote.attachments.length > 0 ? (
                  editedNote.attachments.map(att => (
                    <div key={att.id} className="group relative bg-gray-50 dark:bg-white/[0.02] p-4 rounded-2xl border border-gray-100 dark:border-white/5 hover:border-blue-200 dark:hover:border-indigo-500/30 transition-all flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-gray-900 dark:text-white truncate">{att.name}</p>
                        <p className="text-[10px] text-gray-400 tracking-wide font-medium">{att.size || 'N/A'}</p>
                      </div>
                      <button
                  ...
                  <div className="col-span-full py-12 flex flex-col items-center justify-center bg-gray-50/50 dark:bg-white/[0.01] rounded-3xl border-2 border-dashed border-gray-100 dark:border-white/5">
                    ...
                    <p className="text-sm font-bold text-gray-400">No files attached yet</p>
                    <p className="text-[10px] text-gray-400 mt-1 uppercase tracking-widest">Images, documents, and references</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="xl:col-span-1 space-y-6">
            <div className="p-8 border border-white/60 dark:border-white/5 shadow-sm rounded-[32px] bg-white dark:bg-[#1C1C1E] h-fit">
              <h5 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight mb-6">Linked Context</h5>
              <div className="space-y-4">
                {linkedShot && (
                  <button onClick={() => onNavigateToShot(linkedShot.id)} className="w-full bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/10 flex items-center justify-between group transition-all hover:scale-[1.02] active:scale-[0.98]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Sequence</p>
                        <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-blue-600 transition-colors line-clamp-1">Sc {linkedShot.sceneNumber} • {linkedShot.title}</p>
                      </div>
                    </div>
                ...
                {linkedTask && (
                  <button onClick={() => onNavigateToTask(linkedTask.id)} className="w-full bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/10 flex items-center justify-between group transition-all hover:scale-[1.02] active:scale-[0.98]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Task</p>
                        <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-orange-500 transition-colors line-clamp-1">{linkedTask.title}</p>
                      </div>
                    </div>
                ...
                {!linkedShot && !linkedTask && (
                  <div className="py-8 flex flex-col items-center justify-center bg-gray-50/50 dark:bg-white/[0.01] rounded-2xl border border-dashed border-gray-100 dark:border-white/5">
                    ...
                    <p className="text-xs font-bold text-gray-400">No linked items</p>
                  </div>
                )}
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-gray-100 dark:border-white/5">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="bg-gray-50 dark:bg-white/[0.02] p-3 rounded-xl border border-gray-100 dark:border-white/5">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Last Modified</p>
                    <p className="text-xs font-bold text-gray-700 dark:text-white">{new Date(note.updatedAt).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-white/[0.02] p-3 rounded-xl border border-gray-100 dark:border-white/5">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Created</p>
                    <p className="text-xs font-bold text-gray-700 dark:text-white">{new Date(note.createdAt).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                </div>
              </div>
            </div>
