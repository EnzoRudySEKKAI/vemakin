        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8 pt-10 pb-32">
          {isRetaking && (
            <div className="mb-8 p-4 bg-orange-500/10 rounded-2xl border border-orange-500/20 flex flex-wrap items-center gap-4">
              <span className="font-bold text-orange-600 dark:text-orange-400 text-sm">Schedule Retake:</span>
              <input type="date" value={retakeDate} onChange={e => setRetakeDate(e.target.value)} className="bg-transparent border border-orange-500/30 rounded-lg px-3 py-1.5 text-sm font-bold text-gray-700 dark:text-white" />
              <input type="time" value={retakeTime} onChange={e => setRetakeTime(e.target.value)} className="bg-transparent border border-orange-500/30 rounded-lg px-3 py-1.5 text-sm font-bold text-gray-700 dark:text-white" />
              <button onClick={() => setShowRetakeConfirm(true)} className="px-4 py-1.5 bg-orange-500 text-white rounded-lg font-bold text-xs hover:bg-orange-600">Confirm</button>
            </div>
          )}
          ...
          <div className="flex flex-col gap-8 mb-12 pb-10 border-b border-gray-100 dark:border-white/5">
            ...
            <div className="w-full">
              <span className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] leading-none mb-3 block">Current Status</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:flex lg:flex-wrap items-start gap-x-8 lg:gap-x-16 gap-y-8">
              ...
              <div className="flex flex-col gap-3 min-w-0">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] leading-none">Schedule</span>
                {isEditing ? (
                  <div className="flex flex-col gap-3 max-w-xs">
                    <input type="date" value={editedShot.date} onChange={e => setEditedShot({ ...editedShot, date: e.target.value })} className="bg-transparent border-b border-gray-200 dark:border-white/10 py-1 text-lg font-bold text-gray-900 dark:text-white focus:outline-none" />
                  ...
                  <div className="flex flex-col group py-1.5">
                    <span className="text-xl font-bold text-gray-900 dark:text-white block leading-tight mb-0.5">
              ...
              <div className="min-w-0 lg:flex-1">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] leading-none block mb-3">Location</span>
                {isEditing ? (
                  <input type="text" value={editedShot.location} onChange={e => setEditedShot({ ...editedShot, location: e.target.value })} className="w-full max-w-md bg-transparent border-b border-gray-200 dark:border-white/10 py-1 text-lg font-bold text-gray-900 dark:text-white focus:outline-none" />
                ) : (
                  <div
                  ...
                  >
                    <span className="text-xl font-bold text-gray-900 dark:text-white block leading-tight mb-0.5 truncate w-full">
                      {selectedShot.location}
                    </span>
                    <span className="text-sm font-bold text-emerald-600 dark:text-emerald-500 flex items-center gap-1.5">
            ...
            <div className="xl:col-span-8 space-y-12">
              ...
              <section>
                <div className="flex items-center mb-6">
                  <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Director's Brief</h3>
                </div>
                {isEditing ? (
                  <textarea
                  ...
                  <p className="text-xl font-medium text-gray-700 dark:text-gray-200 leading-[1.6] max-w-3xl">
              ...
              <section className="pt-12 border-t border-gray-100 dark:border-white/5">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center">
                    <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Production Notes ({associatedNotes.length})</h3>
                  </div>
                  <button
                    onClick={() => onAddNote({ title: '', content: '', shotId: selectedShot.id, attachments: [] })}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-500/5 text-blue-600 dark:text-indigo-400 text-xs font-bold hover:bg-blue-500/10 transition-all"
                  >
                ...
                {associatedNotes.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {associatedNotes.map(note => (
                      ...
                      >
                        <h4 className="font-bold text-gray-900 dark:text-white text-lg mb-2 group-hover:text-blue-600 transition-colors">{note.title || "Untitled Note"}</h4>
                  ...
                  <div className="py-12 bg-gray-50/50 dark:bg-white/[0.02] rounded-[32px] border border-dashed border-gray-200 dark:border-white/10 flex flex-col items-center justify-center text-center">
                    <p className="text-sm font-bold text-gray-400">No notes for this shot yet.</p>
                  </div>
                )}
              </section>
            </div>
            ...
            <aside className="xl:col-span-4 lg:sticky lg:top-8 self-start">
              <div className="p-2">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100 dark:border-white/5">
                  <div className="flex items-center">
                    <h3 className="font-bold text-xl text-gray-900 dark:text-white tracking-tight">Camera & Gear</h3>
                  </div>
                  {!isEditing && (
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Checklist</span>
                      <span className={`text-xs font-black ${isChecklistComplete ? 'text-green-600' : 'text-blue-600 dark:text-indigo-400'}`}>
                ...
                {isEditing && (
                  <div className="flex gap-4 border-b border-gray-100 dark:border-white/5 mb-6">
                    <button
                      ...
                      className={`pb-2 text-xs font-bold transition-all border-b-2 ${activeGearTab === 'list' ? 'border-blue-600 dark:border-indigo-500 text-gray-900 dark:text-white' : 'border-transparent text-gray-400'}`}
                    >
                      Assigned
                    </button>
                    <button
                      onClick={() => setActiveGearTab('pool')}
                      className={`pb-2 text-xs font-bold transition-all border-b-2 ${activeGearTab === 'pool' ? 'border-blue-600 dark:border-indigo-500 text-blue-600 dark:text-indigo-400' : 'border-transparent text-gray-400'}`}
                    >
                      Browse Pool
                    </button>
                  </div>
                  ...
                  {(!isEditing || activeGearTab === 'list') && (
                    currentEquipmentIds.length > 0 ? (
                            ...
                            >
                              <div className="flex items-center gap-4 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <p className={`text-base font-bold truncate ${isReady ? 'text-green-700 dark:text-green-300' : 'text-gray-900 dark:text-gray-100'}`}>{item ? (item.customName || item.name) : 'Unknown'}</p>
                      ...
                      <div className="py-12 text-center">
                        ...
                        <p className="text-sm font-bold text-gray-400">No gear assigned.</p>
                      </div>
                    )
                  )}
                    ...
                    <div className="space-y-4">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full bg-gray-100 dark:bg-white/5 border-transparent rounded-2xl pl-12 pr-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                        />
                      </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-4 min-w-0">
                                ...
                                <span className="text-sm font-bold text-gray-700 dark:text-gray-300 truncate">{gear.customName || gear.name}</span>
                              </div>
                              <Plus size={16} className="text-blue-500" />
                            </button>
                          );
                        }) : (
                          <p className="text-center py-8 text-xs font-bold text-gray-400">No matching gear found.</p>
                        )}
                      </div>
                    </div>
                  )}
