...
export const ProjectsManagerView: React.FC<ProjectsManagerViewProps> = ({
    ...
    const saveEditing = () => {
        if (editValue && editingProjectId) {
            onRenameProject(editingProjectId, editValue);
        }
        setEditingProjectId(null);
    };
        ...
        <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
            ...
            <div className="space-y-3">
                {projects.map(project => {
                    ...
                    return (
                        <GlassCard key={project.id} className="p-4 bg-white dark:bg-[#1A1A1D] border-none shadow-sm flex items-center justify-between group">
                                ...
                                {isEditing ? (
                                    <div className="flex-1 mr-4">
                                        <input
                                            ...
                                            className="w-full bg-gray-50 dark:bg-white/5 border border-blue-200 dark:border-blue-500 dark:border-indigo-500/30 rounded-xl px-4 py-2 text-sm font-semibold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:ring-indigo-500/20"
                                            autoFocus
                                        />
                                    </div>
                                ) : (
                                    <div className="flex-1 cursor-pointer" onClick={() => onSelectProject(project.id)}>
                                        <h4 className={`text-base font-semibold ${isCurrent ? 'text-indigo-700 dark:text-blue-400 dark:text-indigo-400' : 'text-gray-900 dark:text-white'}`}>
                                            {project.name}
                                        </h4>
                                        {isCurrent && (
                                            <span className="inline-block mt-1 text-[9px] font-semibold bg-blue-100 dark:bg-blue-500 dark:bg-indigo-500/20 text-blue-600 dark:text-indigo-600 dark:text-blue-300 px-2 py-0.5 rounded-md">
                                                Active workspace
                                            </span>
                                        )}
                                    </div>
