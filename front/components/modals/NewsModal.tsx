   ...
   <GlassCard className="w-full max-w-md bg-white/90 p-0 overflow-hidden shadow-2xl border-white/60"onClick={(e) => e.stopPropagation()}>
    <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white/50">
     <div>
      ...
      <p className="text-[10px] font-semibold text-gray-400 mt-1">Vemakin updates</p>
     </div>
     ...
     {updates.map((update, idx) => (
      <div key={idx} className="relative pl-6 border-l-2 border-gray-100 last:border-0 pb-2">
       ...
       <div className="flex justify-between items-start mb-1">
        <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-md ${idx === 0 ? 'bg-blue-100 text-blue-600 dark:text-indigo-600' : 'bg-gray-100 text-gray-500'}`}>
         v{update.version}
        </span>
        <span className="text-[9px] font-semibold text-gray-400">{update.date}</span>
       </div>
       <h3 className="text-sm font-semibold text-gray-900 mt-2 mb-1">{update.title}</h3>
       <p className="text-xs text-gray-500 leading-relaxed font-medium">{update.description}</p>
      </div>
     ))}
    </div>
    <div className="p-4 bg-gray-50 border-t border-gray-100 text-center">
     <p className="text-[10px] font-semibold text-gray-400">Stay tuned for more updates!</p>
    </div>
   </GlassCard>
  </div>
 );
