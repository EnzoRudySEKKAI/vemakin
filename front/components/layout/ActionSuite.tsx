...
import { SHOOT_DATES, GEAR_DATABASE, CATEGORY_ICONS, POST_PROD_CATEGORIES } from '../../constants.ts';
...
export const ActionSuite: React.FC<ActionSuiteProps> = ({
  ...
  const fromISODate = (isoStr: string) => {
    if (!isoStr) return SHOOT_DATES[0];
  ...
  const [gearForm, setGearForm] = useState({
    name: '',
    serialNumber: '',
    category: '' as keyof typeof GEAR_DATABASE | 'Other' | '',
    brand: '',
    mount: '', // Added mount state
    model: '',
    isOwned: true,
    price: 0,
  ...
  const [taskForm, setTaskForm] = useState({
    ...
    dueDate: toISODate(SHOOT_DATES[SHOOT_DATES.length - 1]),
    description: '',
  ...
  const availableBrands = useMemo(() => {
    if (!gearForm.category || gearForm.category === 'Other') return [];
    const catData = (GEAR_DATABASE as any)[gearForm.category];
    return catData ? Object.keys(catData.brands) : [];
  }, [gearForm.category]);

  const availableMounts = useMemo(() => {
    if (gearForm.category !== 'Lens' || !gearForm.brand) return [];
    const brandData = (GEAR_DATABASE as any).Lens?.brands?.[gearForm.brand];
    if (!brandData) return [];

    // Extract unique mounts from all models of this brand
  ...
  }, [gearForm.category, gearForm.brand]);

  const availableModels = useMemo(() => {
    if (!gearForm.category || gearForm.category === 'Other' || !gearForm.brand) return [];
    const catData = (GEAR_DATABASE as any)[gearForm.category];
    const brandData = catData?.brands?.[gearForm.brand];

    if (!brandData) return [];

    let models = Object.entries(brandData.models);

    // If it's a lens and a mount is selected, filter by mount
    if (gearForm.category === 'Lens' && gearForm.mount) {
      models = models.filter(([_, data]: any) => data.specs?.mount === gearForm.mount);
    }

    return models.map(([name]) => name);
  }, [gearForm.category, gearForm.brand, gearForm.mount]);

  const currentModelSpecs = useMemo(() => {
    if (!gearForm.category || !gearForm.brand || !gearForm.model || gearForm.category === 'Other') return null;
    const catData = (GEAR_DATABASE as any)[gearForm.category];
    const brandData = catData?.brands?.[gearForm.brand];
    const modelData = brandData?.models?.[gearForm.model];
    return modelData?.specs || null;
  }, [gearForm.category, gearForm.brand, gearForm.model]);
      ...
      >
        <div className="bg-white dark:bg-[#1C1C1E] border border-gray-100 dark:border-white/10 shadow-2xl rounded-[32px] sm:rounded-[48px] animate-in slide-in-from-bottom-8 duration-500 relative max-h-[95vh] md:max-h-[92vh] lg:max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-6 sm:px-10 sm:py-8 border-b border-gray-50 dark:border-white/5 flex flex-col gap-6 shrink-0 bg-white dark:bg-[#1C1C1E] z-20">
                ...
                {view !== 'project' && VIEWS.map(v => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[10px] font-bold tracking-wide capitalize ${isActive ? '' : 'hidden sm:inline'}`}>{v.label}</span>
                    </button>
                  )
                })}
              </div>
            ...
            {/* Dynamic Title */}
            <div>
              <p className="text-[9px] font-bold text-blue-500 dark:text-indigo-500 mb-1 tracking-wide capitalize">Create new</p>
              <h3 className="text-2xl sm:text-3xl font-bold tracking-tight capitalize text-gray-900 dark:text-white leading-none">
              ...
              <div className="space-y-5 relative h-full flex flex-col">
                ...
                <div className="rounded-[24px] p-5 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Scene Identity</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 bg-white/70 dark:bg-white/5 border border-white/60 dark:border-white/10 rounded-2xl px-4 py-3.5">
                        <span className="px-2 py-0.5 bg-black/5 dark:bg-white/10 rounded-md text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase">SC</span>
                        <input
                          type="text"
                          value={shotForm.sceneNumber}
                          onChange={e => setShotForm({ ...shotForm, sceneNumber: e.target.value })}
                          className="flex-1 bg-transparent text-base font-bold focus:outline-none text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 min-w-0"
                          placeholder="4C"
                        />
                      </div>
                    </div>
                ...
                <div className="rounded-[24px] p-5 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Schedule</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="relative">
                      <input
                        ...
                        className={`w-full bg-white/70 dark:bg-white/5 border rounded-2xl px-4 py-3.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 dark:ring-indigo-500/20 dark:focus:ring-yellow-400/20 transition-all cursor-pointer appearance-none text-gray-900 dark:text-white ${shotConflict ? 'border-red-200 dark:border-red-500/30' : 'border-white/60 dark:border-white/10'}`}
                      />
                    </div>
                ...
                <div className="rounded-[24px] p-5 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 relative z-20">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-white/70 dark:bg-white/5 border border-white/60 dark:border-white/10 rounded-2xl px-4 py-3.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:focus:ring-yellow-400/20 transition-all text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                  ...
                  {locationSuggestions.length > 0 && (
                    <div className="absolute left-0 right-0 mt-2 mx-5 bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                      <div className="px-4 py-2.5 border-b border-gray-100 dark:border-white/5 bg-gray-50/80 dark:bg-white/5 flex items-center justify-between">
                        <span className="text-[9px] font-bold tracking-wide text-gray-400 dark:text-gray-500 uppercase">Maps suggestions</span>
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {locationSuggestions.map((suggestion, i) => (
                          ...
                          >
                            <span className="text-sm font-bold text-gray-700 dark:text-gray-300 group-hover/loc:text-blue-600 dark:text-indigo-600 dark:group-hover/loc:text-yellow-400 transition-colors truncate pr-2">
                              {suggestion.name}
                            </span>
                ...
                <div className="rounded-[24px] p-5 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Scene Description</span>
                  </div>
                ...
                <div className="rounded-[24px] p-5 bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Equipment</span>
                    </div>
                    <span className="bg-blue-50 dark:bg-yellow-500/10 text-blue-600 dark:text-indigo-600 dark:text-blue-400 dark:text-indigo-400 px-2.5 py-1 rounded-lg text-[10px] font-bold">{shotForm.equipmentIds.length} Selected</span>
                  </div>
                  ...
                  {isCreatingGearInline ? (
                    <div className="p-4 bg-indigo-50 dark:bg-yellow-500/10 border border-indigo-100 dark:border-yellow-500/20 rounded-2xl animate-in fade-in slide-in-from-top-2">
                      <div className="space-y-4">
                        <div>
                          <label className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 block ml-1">Item name / Model</label>
                          <input
                            type="text"
                            value={inlineGearForm.name}
                            onChange={(e) => setInlineGearForm({ ...inlineGearForm, name: e.target.value })}
                            className="w-full bg-white dark:bg-[#2C2C30] border-2 border-gray-100 dark:border-white/10 rounded-[20px] px-4 py-4 text-sm font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 dark:border-indigo-500 dark:focus:border-yellow-400 transition-all placeholder-gray-300 dark:placeholder-gray-500"
                            placeholder="e.g. Red Komodo, Extension Cable..."
                          />
                        </div>
                        <div>
                          <label className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 block ml-1">Category</label>
                          <select
                            value={inlineGearForm.category}
                            onChange={(e) => setInlineGearForm({ ...inlineGearForm, category: e.target.value })}
                            className="w-full bg-white dark:bg-[#2C2C30] border-2 border-gray-100 dark:border-white/10 rounded-[20px] px-4 py-4 text-sm font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 dark:border-indigo-500 dark:focus:border-yellow-400 transition-all cursor-pointer"
                          >
                      ...
                      <div className="flex gap-3 mt-4">
                        <button
                          ...
                          className="flex-1 py-2.5 bg-blue-600 dark:bg-indigo-600 dark:bg-yellow-400 text-white dark:text-gray-900 rounded-xl text-[9px] font-bold tracking-wide shadow-md disabled:opacity-50 hover:bg-blue-700 dark:bg-indigo-700 dark:hover:bg-yellow-500"
                        >
                          Save & Select
                        </button>
                        <button
                          onClick={() => setIsCreatingGearInline(false)}
                          className="px-6 py-2.5 bg-white dark:bg-[#3A3A3E] text-gray-500 dark:text-gray-300 border border-gray-200 dark:border-white/10 rounded-xl text-[9px] font-bold tracking-wide hover:bg-gray-50 dark:hover:bg-[#4A4A4E]"
                        >
                          Cancel
                        </button>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            className="w-full bg-white dark:bg-[#2C2C30] border border-gray-200 dark:border-white/10 rounded-xl pl-9 pr-4 py-2 text-[10px] font-bold tracking-wide text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:focus:ring-yellow-400/20"
                          />
                        </div>
                      </div>
                      <div className="flex gap-1.5 overflow-x-auto pb-2 no-scrollbar">
                        {['All', 'Camera', 'Lens', 'Light', 'Filter', 'Support', 'Grip', 'Monitoring', 'Audio', 'Wireless', 'Drone', 'Props'].map(cat => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-[8px] font-bold tracking-wide whitespace-nowrap border transition-all ${shotGearCategory === cat
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <p className={`text-[10px] font-bold capitalize truncate leading-none mb-0.5 ${isSelected ? 'text-blue-900 dark:text-blue-400 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300'}`}>
                                    {item.customName || item.name}
                                  </p>
                                  <p className="text-[8px] font-bold text-gray-400 dark:text-gray-500 tracking-wide capitalize">{item.category}</p>
                                </div>
                              </div>
                ...
                <div className="pt-4 sticky bottom-0 bg-white dark:bg-[#1C1C1E] flex justify-center pb-6 mt-auto">
                  <button
                    ...
                    className={`w-full sm:w-auto sm:px-24 py-4 rounded-full font-bold text-[12px] tracking-wide text-white shadow-2xl transition-all flex items-center justify-center gap-3 ${!shotForm.title.trim() || !!shotConflict ? 'bg-gray-400 cursor-not-allowed opacity-40' : 'bg-blue-400 dark:bg-indigo-400/60 dark:bg-yellow-400/60 shadow-blue-500 dark:shadow-indigo-500/10 dark:shadow-yellow-400/10 active:scale-[0.98] hover:bg-blue-500 dark:bg-indigo-500 dark:hover:bg-yellow-500 dark:text-gray-900'}`}
                  >
            ...
            {view === 'project' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  <div>
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 block ml-1">Production title</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-gray-50 dark:bg-[#2C2C30] border border-gray-100 dark:border-white/10 rounded-2xl pl-12 pr-4 py-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 dark:ring-indigo-500/10 dark:focus:ring-yellow-400/10 text-gray-900 dark:text-white"
                ...
                <div className="pt-6">
                  <button
                    ...
                    className="w-full py-5 rounded-[24px] font-bold text-[12px] tracking-wide bg-emerald-600 dark:bg-emerald-500 text-white shadow-2xl shadow-emerald-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-40 disabled:grayscale"
                  >
