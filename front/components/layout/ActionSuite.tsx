...
export const ActionSuite: React.FC<ActionSuiteProps> = ({
  ...
  const [taskForm, setTaskForm] = useState({
    title: '',
    type: 'Script' as PostProdTask['type'],
  ...
  const { categories, fetchCatalog } = useGearCatalog();

  React.useEffect(() => {
    if (categories.length === 0) fetchCatalog();
  }, []);

  // -- HANDLERS --

  const handleLocationInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setShotForm({ ...shotForm, location: e.target.value });
  };
  ...
  const handleCommitGear = () => {
    // Map form price to specific fields based on ownership
    const payload = {
      ...gearForm,
      rentalPrice: !gearForm.isOwned ? gearForm.price : 0,
      rentalFrequency: !gearForm.isOwned ? gearForm.frequency : 'day',
      pricePerDay: 0 // Or mapped if we add a rate for owned gear later
    };

    onCommitGear(payload as any);
    onClose();
  };
