    ...
    >
     {filteredTasks.length > 0 ? (
         ...
         >
          <div>
            ...
            <div className="flex items-center gap-3">
             ...
             <div>
              <div className="flex items-center gap-2 mb-1">
               <span className="text-[10px] font-semibold text-gray-400">{task.category}</span>
               <div className={`px-2 py-0.5 rounded-md border text-[9px] font-semibold ${getPriorityColor(task.priority)}`}>
                {task.priority}
               </div>
              </div>
              {task.dueDate && (
           ...
           <div className="mb-6 pl-1">
            <h4 className="text-xl font-semibold text-gray-900 dark:text-white leading-tight mb-2 transition-colors duration-200">
             {task.title}
            </h4>
          ...
          <div className="relative mt-auto"ref={isActiveMenu ? statusMenuRef : null}>
           ...
           >
            <div className="flex items-center gap-2">
             ...
             <span className="">{task.status.charAt(0).toUpperCase() + task.status.slice(1)}</span>
            </div>
            ...
            <div className="absolute left-0 right-0 bottom-full mb-2 bg-white rounded-xl shadow-xl border border-gray-100 p-1.5 z-[2001] animate-in fade-in slide-in-from-bottom-2 duration-200">
             {(['todo', 'progress', 'review', 'done'] as const).map((s) => (
              <button
               ...
               className={`w-full text-left px-3 py-2.5 rounded-lg text-[10px] font-semibold mb-0.5 flex items-center justify-between transition-colors duration-200 ${task.status === s
         ...
         <div className="w-full flex items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-4 min-w-0 flex-1">
           ...
           <div className="min-w-0">
            <h4 className="text-xl font-semibold text-gray-900 dark:text-white transition-colors truncate">
             {task.title}
            </h4>
          ...
          <div className="flex items-center gap-6 shrink-0 ml-auto mr-2">
           ...
           <div className="hidden lg:flex items-center">
            <span className="w-28 flex justify-center px-3 py-1.5 bg-gray-50 dark:bg-white/5 backdrop-blur-md text-gray-400 dark:text-gray-400 text-[10px] font-semibold rounded-xl border border-gray-100 dark:border-white/5 whitespace-nowrap">
             {task.category}
            </span>
           </div>
           ...
           <div className="hidden md:flex items-center">
            <span className={`w-28 flex justify-center px-3 py-1.5 rounded-xl text-[10px] font-semibold border whitespace-nowrap ${getPriorityColor(task.priority)}`}>
             {task.priority}
            </span>
           </div>
           ...
           <div className="flex items-center gap-1">
            <div className="relative"ref={isActiveMenu ? statusMenuRef : null}>
             ...
             >
              <div className="w-2 h-2 rounded-full bg-gray-300 dark:bg-gray-600"/>
              <span className="text-xs font-semibold">{task.status.charAt(0).toUpperCase() + task.status.slice(1)}</span>
             </button>
              ...
              <div className="absolute top-full right-0 mt-2 w-32 bg-white/95 backdrop-blur-xl border border-white rounded-[20px] shadow-xl z-[5000] p-1.5 animate-in fade-in zoom-in-95 duration-200">
               {(['todo', 'progress', 'review', 'done'] as const).map((s) => (
                <button
                 ...
                 className={`w-full text-left px-3 py-2 rounded-xl text-[10px] font-semibold mb-1 transition-all duration-200 ${task.status === s
      ...
      <div className="col-span-full py-24 text-center flex flex-col items-center animate-in zoom-in-95 duration-500">
       ...
       <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Empty pipeline</h3>
       ...
       <button
        onClick={() => onAddTask?.()}
        className="inline-flex items-center gap-3 px-8 py-4 bg-blue-600 dark:bg-[#4E47DD] text-white rounded-2xl font-semibold text-[11px] shadow-xl shadow-blue-100 dark:shadow-indigo-900/20 hover:bg-blue-700 dark:hover:bg-[#5A54E6] active:scale-95 transition-all"
       >
