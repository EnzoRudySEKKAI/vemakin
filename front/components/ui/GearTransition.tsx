      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
       ...
       <div className="space-y-3">
        <div className="flex items-center gap-2.5 px-1">
         ...
         <div>
          <h4 className="text-[11px] font-semibold text-red-600 dark:text-red-400">Drop</h4>
        ...
        <div className="space-y-1.5">
         {gearDelta.toDrop.length > 0 ? gearDelta.toDrop.map(item => {
           ...
           <div key={item.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/5 group/item">
            <div className="flex items-center gap-3 min-w-0">
             ...
             <div className="min-w-0">
              <p className="text-[11px] font-semibold text-gray-900 dark:text-white truncate leading-none mb-1">{item.customName || item.name}</p>
              <p className="text-[9px] font-medium text-gray-400 dark:text-gray-500">{item.category}</p>
             </div>
            </div>
          ...
          <p className="text-[10px] font-semibold text-gray-300 dark:text-gray-600 text-center py-4 border border-dashed border-gray-200 dark:border-white/10 rounded-xl">Everything stays</p>
         )}
        </div>
       </div>
       ...
       <div className="space-y-3">
        <div className="flex items-center gap-2.5 px-1">
         ...
         <div>
          <h4 className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">Add</h4>
        ...
        <div className="space-y-1.5">
         {gearDelta.toAdd.length > 0 ? gearDelta.toAdd.map(item => {
          ...
          return (
           <div key={item.id} className="flex items-center justify-between p-3 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl border border-emerald-100 dark:border-emerald-500/20 group/item">
            <div className="flex items-center gap-3 min-w-0">
             ...
             <div className="min-w-0">
              <p className="text-[11px] font-semibold text-emerald-900 dark:text-emerald-200 truncate leading-none mb-1">{item.customName || item.name}</p>
              <p className="text-[9px] font-medium text-emerald-500 dark:text-emerald-400/70">{item.category}</p>
             </div>
            </div>
          ...
          <p className="text-[10px] font-semibold text-gray-300 dark:text-gray-600 text-center py-4 border border-dashed border-gray-200 dark:border-white/10 rounded-xl">No new gear needed</p>
         )}
        </div>
       </div>
      </div>
