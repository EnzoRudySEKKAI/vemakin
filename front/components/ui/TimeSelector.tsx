...
export const TimeSelector: React.FC<TimeSelectorProps> = ({ label, value, onChange }) => {
  ...
  return (
    <div>
      {label && <label className="text-sm font-bold text-gray-700 dark:text-gray-300 tracking-wide ml-1 mb-2 block">{label}</label>}
      <div className="flex items-center gap-1.5 bg-white/70 dark:bg-white/5 border border-white/60 dark:border-white/10 rounded-2xl p-1.5">
        <div className="relative flex-1 group">
          <select
            ...
            className="w-full appearance-none bg-gray-50 dark:bg-[#3C3C40] border border-transparent rounded-2xl py-3 pl-4 pr-8 text-sm font-bold tracking-wide text-gray-900 dark:text-white focus:outline-none focus:bg-blue-50 dark:focus:bg-yellow-500/10 focus:text-blue-600 dark:text-indigo-600 dark:focus:text-yellow-400 cursor-pointer transition-all"
          >
        ...
        <span className="text-gray-300 dark:text-gray-500 font-bold mb-0.5 text-xs">:</span>
        <div className="relative flex-1 group">
          <select
            value={m}
